# largely based on MONAI generative models - source code added verbatim here and adapted where appropriate to reduce number of dependencies
...
class AutoencoderKL(nn.Module):
    ...
    def __init__(
        ...
        use_checkpointing: bool = False,
        use_convtranspose: bool = False,
        image_size: Sequence[int] | int = None,
        flatten_latent: bool = False,
    ) -> None:
        super().__init__()
        ...
        if flatten_latent is True and image_size is None:
            raise ValueError(
                "If using flatten_latent, must provide fixed input image size to calculate flat dims."
            )
    ...
    def encode(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.flatten_latent:
            h = h.view(h.size(0),-1)
        z_mu = self.quant_mu(h)
        z_log_var = self.quant_log_sigma(h)
...
class GaussAutoencoderKL(nn.Module):
    """
    Autoencoder model with KL-regularized latent space based on
    Rombach et al. "High-Resolution Image Synthesis with Latent Diffusion Models" https://arxiv.org/abs/2112.10752
    and Pinaya et al. "Brain Imaging Generation with Latent Diffusion Models" https://arxiv.org/abs/2209.07162
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_checkpointing:
            ...
            dec_mu = self.decoder_mu(z)
            dec_log_sigma = self.decoder_log_sigma(z)
        return dec_mu, dec_log_sigma
